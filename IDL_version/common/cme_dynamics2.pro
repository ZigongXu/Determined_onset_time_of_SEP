...
if not keyword_set(num) then begin
 ...
 str=''
  for i=0,num-1 do begin
    ...
    if (temp[1] ne 'Invalid' and double(temp[4]) eq 1) then onset[i]=temp[1] else onset[i]='-1'
    ;//this is a case code just for this work-2017.7.21
    travel_time[i]=double(temp[2])
    if (temp[1] ne 'Invalid' and double(temp[4]) eq 1) then release[i]=temp[3] else release[i]='-1'
  endfor
  return,1
end
;-------------------------------
...
;;
;;
pro CME_dynamics2,event_num,result_dir=result_dir,image_dir=image_dir,polynominal=polynominal
...
;
;Modification history
;2017.10.20 a copy of CME_dynamics and modified a little
;
;
...
; read data and analysis
 ...
 if not keyword_set(polynominal) then begin
    ...
    openw,lun_R_sun,'/Users/zigongxu/Documents/work/myWork2_GCS_releasetime/result/2013-2014/CME_start_time.dat',/get_lun,/append
    ...
    Wind_data_name=result_dir+event_num+'/result_wind.dat'
    openr,lun,/get_lun,Wind_data_name
    a=''
    readf,lun,a
    readf,lun,a
  ...
  ;  print,wind_onset_time,wind_travel_time,wind_release_time_TSA 
    SOHO_data_name = result_dir+event_num+'/result_soho.dat'
    openr,lun,/get_lun,soho_data_name
    a=''
    readf,lun,a
    readf,lun,a
  ...
  ;  print,soho_onset_time,soho_travel_time,soho_release_time_TSA
    free_lun,lun
    ;---
    STAele_data_name = result_dir+event_num+'/result_STAelectron.dat'
    STBele_data_name = result_dir+event_num+'/result_STBelectron.dat'
    openr,lun,/get_lun,STAele_data_name
    a=''
    for i =0,1 do begin
      readf,lun,a
    endfor
    ...
    free_lun,lun
    openr,lun,/get_lun,STBele_data_name
    a=''
    for i= 0,1 do begin
      readf,lun,a
    endfor
    ...
    free_lun,lun
    ;--------
    STApro_data_name = result_dir+event_num+'/result_STAproton.dat'
    STBpro_data_name = result_dir+event_num+'/result_STBproton.dat'

    openr,lun,/get_lun,STApro_data_name
    for i= 0,1 do begin
      readf,lun,a
    endfor
    ...
    free_lun,lun
    openr,lun,/get_lun,STBpro_data_name
    a=''
    for i= 0,1 do begin
      readf,lun,a
    endfor
