pro quicklook_STB_proton,event_num,STBLET_data=STBLET_data,STBLET_jul=STBLET_jul,$
    STBHET_data = STBHET_data, STBHET_jul = STBHET_jul,$
...
;print,multi_day
if not keyword_set(STBLET_data) then begin
      if multi_day then begin
        ...
        proton_STB_name_low=file_search(data_dir+event_num+'/particle','H*behind*'+str_doy_name+'*.txt')
         ;print,proton_STB_name_low
        readcol,proton_STB_name_low[0],format=fmt_let,year,day,PLET1,PLET2,PLET3,PLET4,/silent
      endelse
      
      STBlet_jul=doy2jul(year,day) 
      STBLET_data=[transpose(temporary(PLET1)),transpose(temporary(PLET2)),transpose(temporary(PLET3)),transpose(temporary(PLET4))]
      ...
      if not file_test(data_dir+event_num+'/processed_STB_HET.sav') then begin
        ;2017.6.29 maybe this line will be improved latter
          proton_STB_name_high=file_search(data_dir+event_num+'/particle','AeH'+STEREO_high_proton_partof_filename+'.1m')
          fmt_HET='x,f,a,f,a,x,x,x,x,x,x,d,x,d,x,d,x,d,x,d,x,d,x,d,x,d,x,d,x,d,x,d,x'
          readcol,proton_STB_name_high[0],format=fmt_HET,year,month,day,time,P1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,/silent
          jul=julday(month2num(month),day,year,long(strmid(time,0,2)),long(strmid(time,2,2)),0)
          save,filename=data_dir+event_num+'/processed_STB_HET.sav',jul,time,p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11
      ...
      STBHET_jul=jul[area]
      STBHET_data=[transpose(temporary(p1[area])),transpose(temporary(p2[area])),transpose(temporary(p3[area])),transpose(temporary(p4[area])),transpose(temporary(p5[area])),$
...
;help,STBproton_data
;---------------------
yrange=[10.^(floor(alog10(max_except(STBHET_data,except_range_small=1e-10,/min,/nan)))),10.^(ceil(alog10(max(STBLET_data,/nan))))]
cgplot,STBLET_jul,STBLET_data[0,*],/nodata,xstyle=1,ystyle=1,xtickformat='label_date',/ylog,yrange=yrange,title='STB proton',$
  xrange=[any2jul(start_day),any2jul(end_day)],xtitle='time(start from'+start_day+')',charsize=0.8
temp = where(energy_channel eq 1)
print,where(energy_channel eq 1)
help,STBHET_data
if temp[0] ne -1 then begin
    foreach i,where(energy_channel eq 1) do begin
      if i le 3 then cgplot,STBLET_jul,smooth(stBLET_data[i,*],smooth_num,/nan,/edge_truncate),/over,color=mycolor1[i],symsize =0.1,psym =1
      if i gt 3 then cgplot,STBHET_jul,smooth(stBHET_data[i-4,*],smooth_num,/nan,/edge_truncate),/over,color=mycolor1[i],symsize =0.1,psym =1
    endforeach
endif
